...
public class ParkingUserRepositoryTest extends AbstractRepositoryTest {
    ...
    public void findEngagingOwner(){
        ...
        Stream.of(user1, user2, user3, slot1, slot3, reservation1, reservation3, vacantPeriod1)
                .forEach(e -> sharedEntityManager.persist(e));

        ParkingUser resultUser = parkingUserRepository.findEngagingOwner(slot3.getNumber(), TODAY)
                .orElseThrow(() -> new AssertionError("result should not be null"));

        Assert.assertEquals(user3.getId(), resultUser.getId());
    }
}
