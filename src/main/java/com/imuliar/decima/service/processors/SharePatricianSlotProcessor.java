...
public class SharePatricianSlotProcessor extends AbstractUpdateProcessor {
    ...
    protected void doProcess(Update update, Long chatId) {
        ...
        if (getVacantPeriodRepository().hasIntersections(chatId.intValue(), LocalDate.now(), LocalDate.now())) {
            getMessagePublisher().showPopUpNotification(update.getCallbackQuery().getId(), getMsg("alert.slot_already_shared"));
        } else {
    ...
    protected void publishNotificationToCurrentUser(Update update) {
        getMessagePublisher().sendMessage(update.getCallbackQuery().getMessage().getChatId(), getMsg("msg.your_slot_shared"), new InlineKeyboardMarkupBuilder()
