...
public class SaveVacantPeriodProcessor extends AbstractUpdateProcessor {
    ...
    protected void doProcess(Update update, ParkingUser parkingUser, Long chatId) {
        ...
        VacantPeriod vacantPeriod = new VacantPeriod(startDate, endDate, parkingUser);
        ...
        getMessagePublisher().sendSimpleMessageToGroup(String.format("BOT NOTIFICATION:\n[%s](tg://user?id=%d) has shared their slot for the period from %s to %s.",
                parkingUser.toString(), parkingUser.getTelegramUserId(), startDate.toString(), endDate.toString()));
    }
