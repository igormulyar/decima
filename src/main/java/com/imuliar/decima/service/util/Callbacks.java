package com.imuliar.decima.service.util;

/**
 * <p>Store callback strings constants</p>
 *
 * @author imuliar
 * @since 0.0.1
 */
public class Callbacks {

    public static final String YES = "yes_cbk";
    public static final String NO = "no_cbk";
    public static final String TO_BEGINNING = "to_beginning_cbk";
    public static final String FIND_FREE_SLOT = "find_free_slot_cbk";
    public static final String FIND_USER_BY_ENGAGED_SLOT = "find_by_engaged_cbk";
    public static final String SET_AVAILABILITY = "set_availability";
    public static final String SHOW_PLAN = "show_plan";

    public static final String BOOK_SLOT_CALLBACK_TPL = "book_slot:%s";
    public static final String DROP_BOOKING_TPL = "drop_booking:%s";
    public static final String CANCEL_SLOT_SHARING_TPL = "cancel_sharing:%d";
    public static final String SET_FREE_TPL = "set_free:%s:%s";



}
